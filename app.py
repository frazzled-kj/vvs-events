...
class Events(db.Model):
    ...
    is_umbrella_event = db.Column(db.Boolean, default=False)
...
class RoundView(ModelView):
    form_columns = ['event', 'name', 'description', 'rules']
...
class Adjudicators(db.Model):
    ...
    event_id = db.Column(db.Integer, db.ForeignKey('events.id'), nullable=False)
    sub_event_id = db.Column(db.Integer, db.ForeignKey('rounds.id'), nullable=True)
    event = db.relationship('Events', backref=db.backref('adjudicators', lazy=True))
    sub_event = db.relationship('Rounds', backref=db.backref('adjudicators', lazy=True))
class AdjudicatorView(ModelView):
    form_columns = ['name', 'event', 'sub_event', 'email']
...
class Participants(db.Model):
    ...
    sub_event = db.relationship('Rounds', backref=db.backref('participants', lazy=True))

    def __str__(self):
        return self.name
class ParticipantView(ModelView):
    form_columns = ['name', 'event', 'sub_event', 'institute']
...
with app.app_context():
    admin = Admin(app, name='VVS Events', template_mode='bootstrap4', index_view=IndexView())

admin.add_views(EventView(Events, db.session), RoundView(Rounds, db.session), IndividualScoreView(IndividualScores, db.session), ParticipantView(Participants, db.session), AdjudicatorView(Adjudicators, db.session), VenueView(Venues, db.session), DrawView(Draws, db.session), MotionView(Motions, db.session), TeamScoreView(TeamScores, db.session))
...
@app.route('/admin/release_round', methods=['POST'])
def release_round():
    event_name = request.form['event_name']
...
def round_details(event_id, round_id):
    ...
    participants = Participants.query.filter_by(event_id=event_id).all()
    rounds = Rounds.query.filter_by(event_id=event_id).all()
    draws = Draws.query.filter_by(round_id=round_id).all()
    motions = Motions.query.filter_by(round_id=round_id).all()
    individual_leaderboard = IndividualScores.query.filter_by(event_id=event_id).all()
    institutional_leaderboard = TeamScores.query.filter_by(event_id=event_id).all()
    drawmotions = False
    for motion in motions:
        if motion.is_released:
            drawmotions = True
    return render_template('rounds.html', round = round, draws = draws, event=event, adjudicators=adjudicators, motions = motions, participants=participants, rounds=rounds, individual_leaderboard=individual_leaderboard, institutional_leaderboard=institutional_leaderboard, drawmotions = drawmotions)
